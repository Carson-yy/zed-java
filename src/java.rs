    ...
    fn label_for_completion(
        ...
        match completion.kind? {
            ...
            CompletionKind::Constant => {
                ...
                Some(CodeLabel {
                    spans: vec![
                        CodeLabelSpan::literal(new_name, highlight_name),
                        CodeLabelSpan::literal(" : ", None),
            ...
            CompletionKind::Variable | CompletionKind::Field => {
            ...
            CompletionKind::Class | CompletionKind::Interface | CompletionKind::Enum => {
                let name = completion.label.split(" : ").next()?;
                let detail = completion.detail.unwrap();
                let index = detail.rfind(".").unwrap();
                let namespace = &detail[0..index];
                ...
                Some(CodeLabel {
                    spans: vec![
                        ...
                        CodeLabelSpan::literal(" : ", None), 
                        CodeLabelSpan::literal(namespace, None),
                    ],
                    filter_range: (0..name.len()).into(),
                    code,
                })
